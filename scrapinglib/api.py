        ...
        for source in sources:
            try:
                ...
                if self.get_data_state(json_data):
                    if self.debug:
                        logger.success(f"Find movie [{name}] metadata on website '{source}'")
                    break
            except:
                continue
    ...
    def searchAdult(self, number, sources):
        ...
        for source in sources:
            try:
                ...
                if self.get_data_state(json_data):
                    if self.debug:
                        logger.success(f"Find movie [{number}] metadata on website '{source}'")
                    break
            except:
                continue
        ...
        if 'source' in json_data and json_data['source'] == 'javdb':
            ...
            try:
                ...
                if other_json_data is not None and 'cover' in other_json_data and other_json_data['cover'] != '':
                    ...
                    if self.debug:
                        logger.success(f"Find movie [{number}] cover on website '{other_json_data['cover']}'")
            except:
                pass
